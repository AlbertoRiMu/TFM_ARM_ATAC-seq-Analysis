load("~/Escritorio/SIC/results/TGFb2h_kmeans.RData")
load("~/Escritorio/SIC/results/TGFb2h_peaks_index.RData")
library(ggplot2)
library(ggpubr)
library(grid)
...
for(i in 1:10){
  ...
  clusterNum <- c(clusterNum, paste0(rep(i, sum(clust))))
}

numer_Clust = as.numeric(clusterNum) # Change strings of cluster names to numeric to sort them correctly
factor_clust = as.factor(numer_Clust)
